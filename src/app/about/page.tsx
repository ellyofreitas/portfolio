  ...
  return (
    <div className='h-full w-full relative flex flex-col items-start gap-5 overflow-hidden'>
      ...
      <div className='flex flex-col gap-5'>
        <Heading>
          {portfolioConfig.title} And Software <br /> Engineer, Based In{' '}
        ...
        <FramerWrapper y={0} x={100}>
          ...
          <div className='flex flex-col gap-2 pt-2 font-poppins text-xl w-full text-primary max-sm:text-lg'>
            {portfolioConfig.about.bioTipList.map((tip, index) => (
              <p key={index}>{`🔹 ${tip}`}</p>
            ))}
          </div>
          <p className='font-poppins text-xl w-full text-primary max-sm:text-lg pt-2'>
            {portfolioConfig.about.bioFooter}
          </p>
        </FramerWrapper>
      </div>
      <FramerWrapper
        ...
        <div className='w-full h-fit p-2 flex flex-row justify-between gap-7 max-lg:flex-col'>
          {hobbies.map((hobby) => {
            return (
              ...
              >
                <Circle className='h-3 w-3' /> {hobby}
              </div>
            )
          })}
        </div>
